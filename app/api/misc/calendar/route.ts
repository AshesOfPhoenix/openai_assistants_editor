...
const eventList: EventList = {
    calendarId:
        'b03543d0df23f9975a3cba9aa66371dd2ea6dfad6ad0f466b123a8c4b52c7120@group.calendar.google.com',
...
export async function GET(req: NextRequest) {
    const queryParams = [
        ...
        eventList.maxAttendees !== undefined
            ? `maxAttendees=${eventList.maxAttendees}`
            : '',
        eventList.maxResults !== undefined
            ? `maxResults=${eventList.maxResults}`
            : '',
        eventList.orderBy ? `orderBy=${eventList.orderBy}` : '',
        eventList.query ? `q=${eventList.query}` : '',
        eventList.showDeleted !== undefined
            ? `showDeleted=${eventList.showDeleted}`
            : '',
...
export async function POST(req: NextRequest): Promise<any> {
    try {
        const res = await axios.post(
            'https://v1.nocodeapi.com/ash1phoenix/calendar/CLFyYjxtkBpoVgIx/event',
            req.body
        )
